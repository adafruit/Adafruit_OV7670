...
/*=========================================================================
    REGISTERS
    Source: fuyuno sakura
    http://mbed.org/users/mio/code/OV7670_with_AL422B_QQVGA_test/)
    -----------------------------------------------------------------------*/
...
{
 public:
  Adafruit_OV7670(uint8_t addr = OV7670_ADDRESS);
  int8_t begin(void);
  void reset(void);
  void capture(void);
  ...
  uint8_t bufferReadByte(void);
  void assertDataBus(void);
  void deassertDataBus(void);

 private:
  uint8_t ov7670_i2caddr;
